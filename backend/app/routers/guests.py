...
def import_guests(file: UploadFile = File(...), db: Session = Depends(get_db), user: models.User = Depends(get_current_user)):
    try:
        df = pd.read_excel(file.file, keep_default_na=False)
        ...
        for _, row in df.iterrows():
            ...
            if status == "checked_in" and row.get("Giờ vào"):
                try:
                    ...
                    if isinstance(time_val, datetime):
                        ...
                        check_in_time = datetime.strptime(str(time_val), "%d/%m/%Y %H:%M")
                except (ValueError, TypeError) as e:
                    logger.warning(f"Could not parse check-in time '{row.get('Giờ vào')}' for guest '{row.get('Họ tên')}'. Error: {e}")
            ...
            db.add(guest)
        db.commit()
        return {"ok": True, "message": f"Imported {len(df)} records."}
    except Exception as e:
        db.rollback()
    ...
    try:
        ...
        for guest, registered_by_name, registered_by_username in results:
            data_to_export.append({
                ...
                "Ngày đăng ký": guest.created_at.strftime("%d/%m/%Y %H:%M") if guest.created_at else "",
                "Trạng thái": "ĐÃ VÀO" if guest.status == 'checked_in' else "CHƯA VÀO",
                "Giờ vào": guest.check_in_time.strftime("%d/%m/%Y %H:%M") if guest.check_in_time else "",
                "Lý do": guest.reason,
        ...
        headers = {
            'Content-Disposition': f'attachment; filename="guests_export_{datetime.now().strftime("%Y%m%d")}.xlsx"'
        }
...
def clear_guests(db: Session = Depends(get_db)):
    try:
        ...
        db.commit()
        return {"ok": True, "message": "All guest data cleared."}
    except Exception as e:
        db.rollback()
